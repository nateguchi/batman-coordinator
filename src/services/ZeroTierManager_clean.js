...
class ZeroTierManager {
    ...
    async moveZeroTierToNamespace(nsName, chrootPath) {
        try {
            logger.debug('Moving ZeroTier service to namespace + chroot');
            ...
            // Create systemd service for namespaced ZeroTier
            const serviceContent = `[Unit]
Description=ZeroTier One in Network Namespace
After=network.target
Wants=network.target

[Service]
Type=simple
ExecStartPre=/bin/bash -c 'mount -t proc proc ${chrootPath}/proc || true'
ExecStartPre=/bin/bash -c 'mount -t sysfs sysfs ${chrootPath}/sys || true'
ExecStart=/usr/bin/ip netns exec ${nsName} chroot ${chrootPath} /usr/sbin/zerotier-one
ExecStop=/usr/bin/pkill -f "zerotier-one"
ExecStopPost=/bin/bash -c 'umount ${chrootPath}/proc ${chrootPath}/sys 2>/dev/null || true'
Restart=always
RestartSec=5
User=root
...
WantedBy=multi-user.target`;
            ...
            await this.executeCommand(`echo '${serviceContent}' > /etc/systemd/system/zerotier-one-ns.service`);
            await this.executeCommand('systemctl daemon-reload');
            await this.executeCommand('systemctl enable zerotier-one-ns');
            await this.executeCommand('systemctl start zerotier-one-ns');
            ...
            if (nsProcesses.includes('zerotier-one')) {
                logger.debug('✅ ZeroTier successfully running in network namespace');
            } else {
    ...
    async cleanupChrootRouting() {
        try {
            ...
            // Stop and disable the namespaced service
            await this.executeCommand('systemctl stop zerotier-one-ns 2>/dev/null || true');
            await this.executeCommand('systemctl disable zerotier-one-ns 2>/dev/null || true');
            await this.executeCommand('rm -f /etc/systemd/system/zerotier-one-ns.service');
            await this.executeCommand('systemctl daemon-reload');
